...
def setup_venv(venv_pip):
    subprocess.check_call(
        f"{venv_pip} install -U 'numpy<2' torch==2.5.1 torchvision==0.20.1 --index-url https://download.pytorch.org/whl/cu121",
        shell=PLATFORM == "linux",
    )
    if PLATFORM == "windows":
        subprocess.check_call("venv\\Scripts\\python.exe ..\\fix_torch.py")
    subprocess.check_call(
        f"{venv_pip} install -U xformers==0.0.29.post1 --index-url https://download.pytorch.org/whl/cu121",
        shell=PLATFORM == "linux"
    )
    '''
    subprocess.check_call(
