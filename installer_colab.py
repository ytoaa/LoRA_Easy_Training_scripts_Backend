...
def setup_venv(venv_pip):
    subprocess.check_call(
        f"{venv_pip} install -U torch==2.5.0 torchvision==0.20.0 'numpy<2' tensorflow==2.15.0.post1 onnxruntime-gpu==1.19.2 --index-url https://download.pytorch.org/whl/cu124",
        shell=PLATFORM == "linux",
    )
    if PLATFORM == "windows":
